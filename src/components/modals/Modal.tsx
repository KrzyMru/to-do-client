...
const Modal = (props: ModalBaseProps) => {
    ...
    return (
        <div
            className={`fixed inset-0 flex justify-center items-center z-999 ${open ? "visible" : "invisible"} ${render ? "bg-black/20" : "bg-transparent"} focus:outline-none [transition:background-color_350ms]`}
            ref={modalRef}
            role="dialog"
            aria-modal="true"
        ...
        >
            <div
                className={`rounded-md shadow overflow-auto p-6 w-2xl lg:w-4xl max-h-full bg-white dark:bg-gray-800 relative ${render ? "scale-100 opacity-100" : "scale-125 opacity-0"} transition-[width_background-color_scale_opacity] duration-350`}
            ...
            >
                <button
                    ...
                    onClick={handleClose}
                    title={t('modals.baseModal.buttonCloseTitle')}
                    type="button"
                >
                    <svg
