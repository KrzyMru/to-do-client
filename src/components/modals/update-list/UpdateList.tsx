...
import type { UpdateListFormData } from "../../../api/types";

const UpdateList = (props: UpdateListProps) => {
    const { open, onClose, onUpdate, list } = { ...props };
    const { token, setToken } = useContext(UserContext);
    const { register, handleSubmit, control, watch, formState: { errors } } = useForm<UpdateListFormData>({
        defaultValues: {
            id: list.id,
            name: list.name,
    ...
    const onSubmit: SubmitHandler<UpdateListFormData> = async (formData) => {
        try {
            setLoading(true);
            const response = await updateList({ token, setToken, formData });
            onUpdate(response);
            openToast({
                type: 'success',
                title: t('modals.updateList.toast.success.title'),
                text: t('modals.updateList.toast.success.text')
            });
            onClose();
        } catch (e: unknown) {
            openToast({
                type: 'error',
                title: t('modals.updateList.toast.error.title'),
                text: t('modals.updateList.toast.error.text')
            });
        }
        finally {
            setLoading(false);
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="flex p-1 mb-2 justify-center">
                    <div className="flex items-center w-fit rounded-lg p-2 overflow-auto" style={{ backgroundColor: backgroundColor }}>
                        <svg
                            ...
                            viewBox="0 0 24 24"
                            className={`size-[32px] lg:size-[48px] shrink-0 ${name.length !== 0 && 'mr-2'} [transition:height_250ms,width_350ms]`}
                ...
                <div className="flex flex-col pt-4">
                            ...
                            render={({ field: { onChange, value } }) => (
                                <div className="flex flex-col p-1">
                                    <HexColorPicker
                                        color={value}
                                        onChange={onChange}
                                        className="!size-[150px] lg:!size-[200px] !self-center [transition:height_250ms,width_350ms]"
                                    />
                                    ...
                                    icons.map((icon) => (
                                        ...
                                        >
                                            <Icon
                                                type={icon[1]}
                                                color="#4B5563"
                                                className="size-[32px] lg:size-[48px] p-1 [transition:height_250ms,width_350ms]"
                                            />
                                        </button>
                                    ))
                                }
                    ...
                    <Input
                        ...
                        required
                        type="text"
                        label={t('modals.updateList.nameInput.label')}
                        helperText={errors.name ? errors.name.message : t('modals.updateList.nameInput.helperText')}
                        placeholder={t('modals.updateList.nameInput.placeholder')}
                        error={errors.name !== undefined}
                        autoComplete="off"
                        autoFocus
                        {...register("name", { required: t('modals.updateList.nameInput.errorRequired'), minLength: { value: 3, message: t('modals.updateList.nameInput.errorLength') } })}
                    />
                </div>
                <div className="ml-auto w-fit">
                    <Button
                        title={t('modals.updateList.buttonTitle')}
                        type="submit"
                        loading={loading}
                    ...
                    >
                        ...
                        <p className={`text-base lg:text-xl font-mono uppercase ${loadingOrError ? 'text-indigo-100 dark:text-gray-600' : 'text-white'} [transition:font-size_350ms,color_350ms]`}>{t('modals.updateList.buttonText')}</p>
                    </Button>
                </div>
            </form>
        </Modal>
